    ...
    private static class SMTestListener implements TestListener {
        ...
        private void testFailure(Throwable failure, String messageName, String methodName) {
            ...
            try {
                ...
                else if (failure instanceof ComparisonFailure
                    || failure.getClass().getName().equals("org.junit.ComparisonFailure")) {
                    notification = new ComparisonFailureData(
                        ComparisonDetailsExtractor.getExpected(failure),
                        ComparisonDetailsExtractor.getActual(failure)
                    );
                }
